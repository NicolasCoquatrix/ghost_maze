...
# La version 2.1.0 ajoute :
# - Un mode automatique (le personnage termine le jeu automatiquement),
# - Un écran de tutoriel,
# - La gestion des touches 'Entrée', 'Echap', 'Retour arrière' et 'TAB',
...
while keyboard_key == "Confirmer" or keyboard_key == "Recommencer" :
    ...
    for line in range (len(maze)) :
        line_print = ""
        for box in range (len(maze[line])) :
            line_print += maze[line][box]
        print("│ ",line_print, " │")
    for line in range (len(directional_keys)) :
        line_print = ""
        for box in range (len(directional_keys[line])) :
